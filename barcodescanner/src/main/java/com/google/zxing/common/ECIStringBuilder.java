 ...
 */
 ...
 public final class ECIStringBuilder {
   ...
   public ECIStringBuilder(int initialCapacity) {
     ...
     rawData = new ArrayList<Byte>(initialCapacity);
   }
   ...
   public void append(String value) {
     ...
     byte[] bytes = value.getBytes(currentCharset);
     for (byte b : bytes) {
       rawData.add(b);
     }
   }
   ...
   /**
    * Append the string repesentation of {@code value} (short for
    * {@code append(String.valueOf(value))})
    *
     ...
     if (currentCharset.equals(StandardCharsets.ISO_8859_1)) {
       if (currentBytes.length() > 0) {
         if (result == null) {
           result = currentBytes;
           rawData.clear();
           for (byte b : currentBytes.toString().getBytes(StandardCharsets.ISO_8859_1)) {
             rawData.add(b);
           }
   ...
   /**
    * Appends the characters from {@code value} (unlike all other append methods of
    * this class who append bytes)
    *
   ...
   public void appendCharacters(StringBuilder value) {
     encodeCurrentBytesIfAny();
     result.append(value);
     byte[] bytes = value.toString().getBytes(currentCharset);
     for (byte b : bytes) {
       rawData.add((byte) b);
     }
   }
   ...
   /**
    * Short for {@code toString().length()} (if possible, use {@link #isEmpty()}
    * instead)
    *
   ...
   public byte[] getRawBytes() {
     byte[] byteArray = new byte[rawData.size()];
     int i = 0;
     for (Byte b : rawData) {
       byteArray[i++] = b;
     }
