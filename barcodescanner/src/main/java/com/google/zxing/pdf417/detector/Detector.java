 ...
 */
 ...
 public final class Detector {
   ...
   // if we set the value too low, then we don't detect the correct height of the
   // bar if the start patterns are damaged.
   // if we set the value too high, then we might detect the start pattern from a
   // neighbor barcode.
   private static final int SKIPPED_ROW_COUNT_MAX = 80;
   // A PDF471 barcode should have at least 3 rows, with each row being >= 3 times
   // the module width.
   // Therefore it should be at least 9 pixels tall. To be conservative, we use
   ...
   /**
    * <p>
    * Detects a PDF417 Code in an image. Checks 0, 90, 180, and 270 degree
    * rotations.
    * </p>
    *
    * @param image    barcode image to decode
    * @param hints    optional hints to detector
    * @param multiple if true, then the image is searched for multiple codes. If
    *                 false, then at most one code will
   ...
   public static PDF417DetectorResult detect(BinaryBitmap image, Map<DecodeHintType, ?> hints, boolean multiple)
     ...
     // TODO detection improvement, tryHarder could try several different luminance
     // thresholds/blackpoints or even
     ...
     BitMatrix flippedMatrix = flipHorizontally(originalMatrix);
     for (int rotation : ROTATIONS) {
       BitMatrix bitMatrix = applyRotation(flippedMatrix, rotation);
   ...
   private static BitMatrix flipHorizontally(BitMatrix matrix) {
     int width = matrix.getWidth();
     int height = matrix.getHeight();
     BitMatrix flipped = new BitMatrix(width, height);
 
     for (int y = 0; y < height; y++) {
       for (int x = 0; x < width; x++) {
         if (matrix.get(x, y)) {
           flipped.set(width - x - 1, y); // Mirror pixels horizontally
         }
       }
     }
     return flipped;
   ...
   private static List<ResultPoint[]> detect(boolean multiple, BitMatrix bitMatrix) {
     ...
     while (row < bitMatrix.getHeight()) {
       ...
       if (vertices[0] == null && vertices[3] == null) {
         ...
         // we didn't find a barcode starting at the given column and row. Try again from
         // the first column and slightly
       ...
       // if we didn't find a right row indicator column, then continue the search for
       // the next barcode after the
   ...
   private static ResultPoint[] findRowsWithPattern(BitMatrix matrix,
     ...
     if (found) {
       ...
       for (; stopRow < height; stopRow++) {
         ...
         // a found pattern is only considered to belong to the same barcode if the start
         // and end positions
         // don't differ too much. Pattern drift should be not bigger than two for
         // consecutive rows. With
         // a higher number of skipped rows drift could be larger. To keep it simple for
         // now, we allow a slightly
   ...
   private static int[] findGuardPattern(BitMatrix matrix,
     ...
     // if there are black pixels left of the current pixel shift to the left, but
     // only for MAX_PIXEL_DRIFT pixels
   ...
   private static float patternMatchVariance(int[] counters, int[] pattern) {
     ...
     // We're going to fake floating-point math in integers. We just need to use more
     // bits.
     // Scale up patternLength so that intermediate values below like scaledCounter
     // will have
